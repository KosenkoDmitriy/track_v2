...
class ApplicationController < ActionController::Base
  ...
  def men
    ...
    products = find_products_by_taxons(%w(Men Tops Bottoms Accessories))
  ...
  def women
    ...
    products = find_products_by_taxons(%w(Women Tops Bottoms Accessories))
  ...
  def redirect_admin_user
    ...
    unless defined?(current_alchemy_user[:id])
      if request.path.index('admin').to_s && !request.path.index('login').to_s
      ...
      if optionTypeId
        product.variants.each do |var|
          var.option_values.each do |opt|
            next unless opt.option_type_id == optionTypeId
            next if colors.include?(opt.presentation)
            ...
            mainImage = var.images[0] if mainImage.nil?
